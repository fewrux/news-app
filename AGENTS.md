...
- Every PR carries a Plane issue link. Product-surface PRs carry browser evidence on that Plane issue (posted in `/verify` before the draft PR). Operator-surface PRs record an explicit waiver in the reviewer artifact. One PR per lane — never mix product and operator paths (ADR-0006).
