...
| AC-11 | First real run flags the known pre-existing drift: `sdlc.yaml.instructions.hooks.registry` `.sh` mismatch, `integrations.plane.mappings` pseudo-YAML, the legacy `plane_issue:` field on INT-0001 (per ADR-0002). | The first run's report contains those three finding ids. They become the next three cleanup intents.                                                              |
